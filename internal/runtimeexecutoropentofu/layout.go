// Package runtimeexecutoropentofu is the standard StackKits runtime executor
// (ADR-0045): it realizes one module's CUE-generated OpenTofu root with the
// packaged tofu binary and an offline provider mirror, keeps OpenTofu state in
// that root, and verifies the result with the native Verify semantics of the
// module. The native Compose executor in runtimeexecutorlocal stays the
// fallback behind the compose target (S-F).
