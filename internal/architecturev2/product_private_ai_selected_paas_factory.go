...
const productPrivateAISelectedPaaSAdapterID = "stackkits-private-ai-selected-paas"

type productPrivateAISelectedPaaSFactory struct {
...
func NewProductPrivateAISelectedPaaSRegistration(
	...
	if runtimeVersion == "" || runtimeVersion != strings.TrimSpace(runtimeVersion) ||
		...
		return ProductRuntimeOwnerRegistration{}, errors.New("PrivateAI selected-PaaS product registration requires runtime version, exact adapter identity, and operations owner")
	}
	return ProductRuntimeOwnerRegistration{
		Selector: productPrivateAISelectedPaaSSelector(runtimeAdapterRef, runtimeAdapterModuleRef),
		Factory: &productPrivateAISelectedPaaSFactory{
...
func (f *productPrivateAISelectedPaaSFactory) PrepareRuntimeOwner(request ProductRuntimeOwnerRequest) (runtimeexecutor.Executor, error) {
	if f == nil || strings.TrimSpace(f.runtimeVersion) == "" || strings.TrimSpace(f.runtimeAdapterRef) == "" ||
		...
		return nil, errors.New("PrivateAI selected-PaaS product factory is not initialized")
	}
	target := cloneProductRuntimeTarget(request.Target)
	health := cloneProductHealthTargets(request.HealthTargets)
	selector := productPrivateAISelectedPaaSSelector(f.runtimeAdapterRef, f.runtimeAdapterModuleRef)
	if productRuntimeOwnerSelectorForTarget(target) != selector || target.RuntimeAdapter == nil ||
		...
		return nil, errors.New("PrivateAI selected-PaaS product factory requires one exact channel-bound workload, adapter, and health contract")
	}
	moduleHealthIndex := -1
	for index, requirement := range health {
		if !productHealthTargetsRuntime(requirement, target) {
			return nil, errors.New("PrivateAI selected-PaaS product factory received Health outside its exact runtime authority")
		}
		if requirement.TargetKind == "module" {
			if moduleHealthIndex >= 0 {
				return nil, errors.New("PrivateAI selected-PaaS product factory received more than one module Health contract")
			}
			moduleHealthIndex = index
		}
	}
	if moduleHealthIndex < 0 {
		return nil, errors.New("PrivateAI selected-PaaS product factory requires its exact module Health contract")
	}
	identity, err := productRuntimeOwnerAdapterIdentity(productPrivateAISelectedPaaSAdapterID, f.runtimeVersion, target, health)
	if err != nil {
		return nil, err
	}
...
func productPrivateAISelectedPaaSSelector(runtimeAdapterRef, runtimeAdapterModuleRef string) ProductRuntimeOwnerSelector {
	return ProductRuntimeOwnerSelector{
		OwnerKind: "module", OwnerRef: "stackkits-private-ai-runtime",
		ProviderRef: "stackkits-private-ai", ModuleRef: "stackkits-private-ai-runtime", UnitRef: "private-ai",
		RuntimeKind: "container", RuntimeDelivery: sharedApplicationAdapterRuntimeDelivery, RuntimeEngine: "docker", WorkloadRef: "ai",
		RuntimeAdapterRef: runtimeAdapterRef, RuntimeAdapterModuleRef: runtimeAdapterModuleRef,
	}
}

var _ ProductRuntimeOwnerFactory = (*productPrivateAISelectedPaaSFactory)(nil)
