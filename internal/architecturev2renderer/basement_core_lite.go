...
const basementCoreLiteTerramateSchema = `stackkit.basement-core-lite-terramate/v1|artifact-revision:4|runtime-listeners:catalog-bound,direct-loopback-only|engine:terramate|underlay:opentofu|outputs:main.tf,stack.tm.hcl,terramate.tm.hcl|execution-instance:node-local|credentials:none|cloud:none|coolify:omitted|listener-site-address:inventory-bound|acme-leaf-duration:24h-renew-before6h-health-grace10m`
