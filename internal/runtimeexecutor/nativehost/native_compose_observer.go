...
type osNativeComposeRuntime struct {
	...
	// cloudServerBinary overrides the stackkit-server source of the Cloud
	// operations; nil keeps the running release's.
	cloudServerBinary stackKitServerBinarySource
}
...
func (r *osNativeComposeRuntime) cloud(moduleRef string) (*osCloudCoreOperations, error) {
	...
	if r.cloudServerBinary != nil {
		cloud.serverBinary = r.cloudServerBinary
	}
	return cloud, nil
}
