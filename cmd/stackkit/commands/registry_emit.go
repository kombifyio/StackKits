...
//
// The committed Git/CUE tree feeds both the runtime catalog
// (registry_snapshot.json) and the generated CUE files.
//
// Emission sources:
//   - #ToolCatalog: snapshot.Tools from the committed snapshot.
...
refresh it first via 'registry bake-from-cue'), renders
...
func runRegistryEmitCUE(_ *cobra.Command, _ []string) error {
	...
	if snap.SchemaVersion != registry.SnapshotVersion {
		return fmt.Errorf("snapshot schema version mismatch: got %d, expected %d -- refresh with `stackkit registry bake-from-cue`",
