...
const (
	publishedV09CurrentVersion  = "v0.9.0"
	publishedV010UpgradeVersion = "v0.10.0"
	publishedV09ArchiveSHA256   = "6d969b7fbe672a596df4226db4e695d3d4f51a75d8edfc8434f1b37201fef423"
	publishedV09IndexSHA256     = "0dd20f30dfaad350cc5fb8077d68f99f2df8dcdb673817ee4aefdfe4aff5e587"
)

// inspectPublishedV09CurrentGeneration reads the applied v0.9 generation with
// the exact attested v0.9 binary that authored it. The v0.10 authority remains
// strict for all target resolution and mutation.
func inspectPublishedV09CurrentGeneration(
	ctx context.Context,
	workspace string,
...
func inspectPublishedV09UpgradeBridge(
	ctx context.Context,
	workspace string,
	requestedSpec string,
	kit string,
	...
	if !isPublishedV09CompatibilityTarget(kit, target) {
		return publicUpgradeBridge{}, nil
	}
	platform := currentReleasePlatform()
	installDir := filepath.Join(
		workspace, ".stackkit", "releases", kit, publishedV09CurrentVersion,
	...
	err := (releaseindex.Installer{
		...
		return proof.Inspect(func(
			...
			if err := validatePublishedV09Receipt(current, kit, platform); err != nil {
				return err
			}
			receipt = current
			return nil
	...
	err = withPublicUpgradeInstalledExecutable(ctx, receipt, func(binary string) error {
		...
		if runner == nil {
			return errors.New("published v0.9 bridge requires an execution runner")
		}
		...
		if runErr != nil {
			return fmt.Errorf("run attested v0.9 plan proof: %w", runErr)
		}
		if err := decodeUpgradeExactJSON(rawPlan, &bridge.Current); err != nil {
			return fmt.Errorf("decode attested v0.9 plan proof: %w", err)
		}
		if err := validateUpgradePlanInspection(bridge.Current, "published v0.9 current"); err != nil {
			return err
		}
		...
		if runErr != nil {
			return fmt.Errorf("run attested v0.9 offline verification: %w", runErr)
		}
		...
		if runErr != nil {
			return fmt.Errorf("run attested v0.9 live verification: %w", runErr)
		}
		...
		if liveVerifyErr != nil {
			return fmt.Errorf("validate attested v0.9 live verification: %w", liveVerifyErr)
		}
...
func isPublishedV09CompatibilityTarget(
	kit string,
	target releaseindex.Resolution,
) bool {
	return strings.TrimSpace(kit) != "" &&
		target.Asset.Kit == kit &&
		target.Asset.Version == publishedV010UpgradeVersion &&
...
func validatePublishedV09Receipt(
	...
	if receipt.SchemaVersion != releaseindex.ReceiptSchemaVersion ||
		receipt.Kit != kit ||
		receipt.Version != publishedV09CurrentVersion ||
		receipt.Channel != releaseindex.ChannelStable ||
		receipt.Platform != platform ||
		receipt.ArchiveSHA256 != publishedV09ArchiveSHA256 ||
		receipt.IndexSHA256 != publishedV09IndexSHA256 ||
		strings.TrimSpace(receipt.InstallDir) == "" {
		return errors.New("installed release is not the attested published v0.9 stable source")
	}
	return nil
}
