...
func TestGenerateCommand_BaseKitDefaultSpecGoldenPath(t *testing.T) {
	...
	assert.Contains(t, mainTF, `stackkit_dynamic_middleware_enabled = local.stackkit_traefik_enabled || local.base_hub_uses_dynamic_protection`)
	...
	assert.Contains(t, mainTF, `coolify_api_endpoint              = "http://127.0.0.1:8000"`)
	assert.Contains(t, mainTF, `"${local.coolify_api_endpoint}/api/health"`)
	assert.Contains(t, mainTF, `"${local.coolify_api_endpoint}/health"`)
	...
	assert.Contains(t, mainTF, `STACKKIT_PROXY_DOCKER_ENDPOINT="tcp://host.docker.internal:$${STACKKIT_PROXY_DOCKER_ENDPOINT##*:}"`)
	assert.Contains(t, mainTF, `--providers.docker.endpoint=`)
	assert.Contains(t, mainTF, `DOCKER_HOST="${var.docker_host}" docker compose -f "$PROXY_COMPOSE" up -d`)
...
func TestDokployBootstrapPersistsHeadlessAPIKeyConfig(t *testing.T) {
	...
	assert.Contains(t, mainTF, `name  = "node:22-alpine"`)
