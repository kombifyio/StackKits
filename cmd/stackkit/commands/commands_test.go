...
func TestGenerateCommand_BaseKitDefaultSpecGoldenPath(t *testing.T) {
	...
	assert.Contains(t, mainTF, `base_hub_auth_middleware = var.enable_tinyauth && !local.base_hub_bootstrap_open ? "tinyauth@docker" : ""`)
	assert.Contains(t, mainTF, `base_hub_unprotected     = var.enable_dashboard && (local.base_hub_auth_middleware == "")`)
