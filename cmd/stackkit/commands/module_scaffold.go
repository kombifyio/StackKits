...
func init() {
	...
	moduleScaffoldCmd.Flags().StringVar(&moduleScaffoldCatalog, "catalog", "", "Compiled CUE catalog.json for imageSource references")
...
func runModuleScaffold(cmd *cobra.Command, args []string) error {
	...
	var catalog []byte
	if moduleScaffoldCatalog != "" {
		catalog, err = os.ReadFile(moduleScaffoldCatalog)
		if err != nil {
