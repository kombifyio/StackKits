...
export const kits: Kit[] = [
  {
    ...
    description:
      'BaseKit is the verified beta single-environment StackKit. It bootstraps Docker, packaged OpenTofu, identity, dashboard, and the platform baseline on a fresh Ubuntu VM or host, and exposes everything over browser-native `*.home.localhost` links. Current evidence proves the local fallback path plus auth/setup guards; the Cubi/Coolify-managed L3 path remains a documented blocker.',
    ...
    services: ['Coolify baseline + local routing fallback', 'PocketID', 'TinyAuth', 'Homepage dashboard', 'Uptime Kuma', 'Vaultwarden', 'Immich (Photos)', 'stackkit-server (Node Hub API)'],
