#!/bin/sh
# =============================================================================
# StackKits Cloud Installer — native v2 host preparation and intent setup.
# =============================================================================
...
#   3. Initialize the canonical cloud-kit StackSpec
...
#                          Optional Cloud Kit service profile: default or admin-only
...
STACKKIT_CONTEXT_VALUE="${STACKKIT_CONTEXT:-${KOMBIFY_CONTEXT:-cloud}}"
...
info "Step 1/4 -- Installing stackkit CLI + cloud-kit"
...
# --- Step 3: Initialize cloud-kit ----------------------------------------------

info "Step 3/4 -- Initializing cloud-kit"

mkdir -p "$HOMELAB_DIR"
cd "$HOMELAB_DIR"

set -- init cloud-kit --non-interactive
...
ok "  cloud-kit initialized in $HOMELAB_DIR"
...
if [ "${KOMBIFY_CONTEXT:-cloud}" = "cloud" ] || { [ -f /etc/kombify/context ] && [ "$(cat /etc/kombify/context 2>/dev/null)" = "cloud" ]; }; then
  NETWORK_ENV="cloud"
fi
...
echo "    4. Open ${DASH_URL} and click Protect Base Hub after owner setup"
echo ""
