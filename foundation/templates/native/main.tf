# =============================================================================
# __KIT_DISPLAY__ - Native/Bare-Metal Deployment (No Containers)
# =============================================================================
...
variable "pocketid_static_api_key" {
  type        = string
  description = "PocketID v2 STATIC_API_KEY (admin bootstrap token; persisted by stackkit generate)"
  sensitive   = true
  default     = ""
}
...
variable "pocketid_encryption_key" {
  type        = string
  description = "PocketID v2 ENCRYPTION_KEY (>=16 raw bytes; persisted by stackkit generate)"
  sensitive   = true
  default     = ""
}
...
variable "tinyauth_session_secret" {
  type        = string
  description = "TinyAuth session secret; generated when omitted"
  default     = ""
  sensitive   = true
...
variable "tinyauth_oauth_whitelist" {
  type        = string
  description = "Optional TinyAuth OAuth email allowlist. Empty means PocketID is the user source of truth."
  default     = ""
}
...
locals {
  ...
  pocketid_app_url_effective        = var.pocketid_app_url != "" ? var.pocketid_app_url : "http://${local.domains.id}"
  pocketid_internal_oidc_origin     = "http://127.0.0.1:3082"
  pocketid_encryption_key_effective = var.pocketid_encryption_key != "" ? var.pocketid_encryption_key : random_password.pocketid_encryption_key.result
  ...
  content  = <<-YAML
    ...
    http:
      routers:
        ...
        ${var.enable_pocketid ? "  rule: \"Host(`${local.domains.id}`)\"" : ""}
      ...
      ${var.enable_tinyauth ? "        - \"X-User\"" : ""}
      ${var.enable_tinyauth ? "        - \"X-Email\"" : ""}
      ${var.enable_tinyauth ? "        - \"remote-user\"" : ""}
      ${var.enable_tinyauth ? "        - \"remote-sub\"" : ""}
      ${var.enable_tinyauth ? "        - \"remote-name\"" : ""}
      ${var.enable_tinyauth ? "        - \"remote-email\"" : ""}
      ${var.enable_tinyauth ? "        - \"remote-groups\"" : ""}
  YAML
}
...
resource "null_resource" "tinyauth_install" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      ARCH_DL=$(cat ${local.install_dir}/.arch)
      TINYAUTH_VERSION="5.0.7"

      # Download TinyAuth binary
      curl -sSL "https://github.com/tinyauthapp/tinyauth/releases/download/v$${TINYAUTH_VERSION}/tinyauth-$${ARCH_DL}" \
...
resource "local_file" "tinyauth_service" {
  ...
  content  = <<-INI
    ...
    # Environment
    Environment=TINYAUTH_SERVER_PORT=3080
    Environment=TINYAUTH_APPURL=${var.tinyauth_app_url}
    Environment=TINYAUTH_AUTH_USERS=${var.tinyauth_users}
    Environment=TINYAUTH_DATABASE_PATH=${local.data_dir}/tinyauth/tinyauth.db
    Environment=TINYAUTH_RESOURCES_PATH=${local.data_dir}/tinyauth/resources
    %{if var.tinyauth_oidc_enabled}
    Environment=TINYAUTH_OAUTH_AUTOREDIRECT=pocketid
    Environment=TINYAUTH_OAUTH_WHITELIST=${var.tinyauth_oauth_whitelist}
    Environment=TINYAUTH_OAUTH_PROVIDERS_POCKETID_CLIENTID=${var.tinyauth_oidc_client_id}
    Environment=TINYAUTH_OAUTH_PROVIDERS_POCKETID_CLIENTSECRET=${var.tinyauth_oidc_client_secret}
    Environment=TINYAUTH_OAUTH_PROVIDERS_POCKETID_AUTHURL=${var.tinyauth_oidc_issuer}/authorize
    Environment=TINYAUTH_OAUTH_PROVIDERS_POCKETID_TOKENURL=${local.pocketid_internal_oidc_origin}/api/oidc/token
    Environment=TINYAUTH_OAUTH_PROVIDERS_POCKETID_USERINFOURL=${local.pocketid_internal_oidc_origin}/api/oidc/userinfo
    Environment=TINYAUTH_OAUTH_PROVIDERS_POCKETID_REDIRECTURL=${var.tinyauth_app_url}/api/oauth/callback/pocketid
    ...
    Environment=TINYAUTH_OAUTH_PROVIDERS_POCKETID_INSECURE=false
    %{endif}
...
resource "null_resource" "tinyauth_start" {
  count      = var.enable_tinyauth ? 1 : 0
  depends_on = [null_resource.tinyauth_install, local_file.tinyauth_service, null_resource.traefik_start, null_resource.pocketid_tinyauth_oidc_client]
...
resource "local_file" "pocketid_service" {
  ...
  lifecycle {
    precondition {
      condition     = !var.enable_pocketid || trimspace(var.pocketid_static_api_key) != ""
      error_message = "pocketid_static_api_key must be generated by stackkit generate or supplied explicitly before enabling PocketID."
    }

    precondition {
      condition     = !var.enable_pocketid || length(local.pocketid_encryption_key_effective) >= 16
      error_message = "pocketid_encryption_key must be generated by stackkit generate or supplied explicitly before enabling PocketID."
    }
  }
}
...
resource "null_resource" "pocketid_tinyauth_oidc_client" {
  ...
  depends_on = [null_resource.pocketid_start]
}
  ...
  content  = <<-HTML
    ...
    <body>
      ...
      <main>
        ...
        ${var.enable_dashboard && !(var.enable_tinyauth && var.protect_base_hub) ? "<div class=\"banner\"><span class=\"banner-icon\">&#9888;</span><div class=\"banner-text\"><strong>This page is currently unprotected.</strong> Base stays intentionally open during bootstrap. Set up PocketID first, then enable Base Hub protection in the interface to move Base behind TinyAuth.</div></div>" : ""}
        <div class="hdr">
...
output "pocketid_url" {
  ...
  value       = var.enable_pocketid ? "http://${local.domains.id}" : null
}

output "dashboard_url" {
  description = "Dashboard URL"
  value       = var.enable_dashboard ? "http://${local.domains.base}" : null
}
...
output "credentials" {
  ...
  value       = var.enable_tinyauth ? "TinyAuth credentials are generated by stackkit generate and written to sensitive output." : null
}
...
output "architecture_summary" {
  ...
  value       = <<-EOT
    =====================================================================
      __KIT_DISPLAY_UPPER__ - NATIVE/BARE-METAL DEPLOYMENT (No Containers)
    =====================================================================
      ...
      SERVICES:
        ${var.enable_pocketid ? "OK" : "--"} Pocket ID   -> http://${local.domains.id}
        ${var.enable_tinyauth ? "OK" : "--"} TinyAuth    -> http://${local.domains.auth}
           Credentials: generated by stackkit generate
        ${var.enable_traefik ? "OK" : "--"} Traefik     -> http://${local.domains.traefik}
        OK Whoami      -> http://${local.domains.whoami}
        ${var.enable_dashboard ? "OK" : "--"} Dashboard   -> http://${local.domains.base}
