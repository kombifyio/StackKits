...
Open Node Hub first. `http://base.home.localhost` is the local first-setup entrypoint, but local-first does not mean anonymously open by default: when TinyAuth is enabled, protected Node Hub routes and default L3 application routes must reject anonymous access instead of returning a successful page. L3 services may be public or unauthenticated only when the StackSpec/module access policy explicitly configures that exposure. After authenticating through the generated TinyAuth/PocketID path, Node Hub starts with the first-run checklist, then links to Homepage, status, app platforms, and the public service how-to guides for the enabled services on that node.
