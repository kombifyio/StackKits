...
Basement Kit and Cloud Kit are the current stable one-click paths; the unpinned public latest is `v0.5.2` (Coolify readiness patch over the `v0.5.1` Cloud Kit graduation release). Unreleased kit definitions stay outside the public install surface until their release matrices graduate. Product-bundled L3 applications are PaaS-intended by default; Basement Kit evidence must prove owner/passkey activation, selected-PaaS setup, protected app health, retryable L3 setup actions, and seeded content only when demo data is explicitly enabled. User-installed apps outside that path are state-unmanaged.
