...
The effect is that CUE modules can declare contracts (`requires`, `provides`, `contexts` — 14 modules, see `docs/CUE-AUDIT-AND-PLAN.md`) but the Go CLI does not enforce them. A `stackkit.yaml` that violates a contract can produce a deploy artifact that then fails at runtime.

The CUE-AUDIT (2026-02-23 → Phase 1–2 complete 2026-04-07) identified three missing components:
...
- CUE Go API has a learning curve; team needs to build fluency. Mitigation: CUE-AUDIT already has the concept work done; Phase 3–5 is mostly binding, not design.
...
- CUE-AUDIT Phase 3 tickets (Q2/2026 priority 1)
- CUE-AUDIT Phase 5 tickets (Q3/2026 priority 2)
...
- [ARCHITECTURE_V6.md §2](../ARCHITECTURE_V6.md) — CUE Decision Logic
- [ADR-0009](./ADR-0009-three-tier-provisioning.md) — Three-Tier Provisioning (Tier 1 depends on this binding)
- [CUE-AUDIT-AND-PLAN.md](../CUE-AUDIT-AND-PLAN.md) — Phase 3–5 implementation plan
