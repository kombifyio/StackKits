...
**Supersedes nothing**; clarifies + enforces ADR-0012 (`sk_layer`) and the current architecture contract across DB, YAML, Go, TS, and UI.
**Pairs with:** kombify-DB migration 000084, kombify-Administration spec 2026-04-27-admin-stackkit-lifecycle-ui-design.md.

## Context

ADR-0012 introduced `sk_layer` with the canonical layer slugs `foundation` / `platform` / `application` (+ `hardening`, `observability`). The current architecture contract keeps the same names for the hardening baseline and Base Kit application scope.
...
ADR-0012 and the architecture contract say `application` (singular, matching `foundation` / `platform`). Plural breaks the parallelism.
...
- Future "what does Layer X mean?" questions point to [docs/ARCHITECTURE.md](../ARCHITECTURE.md).
...
- `login-gateway` placement is a **separate** drift, intentionally not touched here. It moves a real selection between layers, not just renames a key. Tracked as a follow-up.
