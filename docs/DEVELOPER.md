...
- TinyAuth: http://auth.stack.local (credentials are generated by `stackkit generate`)
...
mise run test:cue
# or run the direct cue vet commands from the commit checklist below

# CUE module binding gate
...
bash modules/traefik/tests/integration_test.sh
...
- Check generated `deploy/providers.tf` and per-module `deploy/*.tf` fragments.

### API server issues

- `STACKKITS_API_KEY` is required for non-local server startup. For local-only debugging, pass `--allow-unauthenticated` or set `STACKKITS_ALLOW_UNAUTHENTICATED=true`.
- Browser CORS is disabled unless `--cors-origins` or `STACKKITS_CORS_ORIGINS` is set. Wildcard CORS requires `--allow-wildcard-cors` or `STACKKITS_ALLOW_WILDCARD_CORS=true`.
...
| `STACKKITS_API_KEY` | API key for server auth | required for non-local server startup | `--api-key` flag |
| `STACKKITS_ALLOW_UNAUTHENTICATED` | Explicit local-only unauthenticated API override | `false` | `--allow-unauthenticated` flag |
| `STACKKITS_CORS_ORIGINS` | Comma-separated browser CORS allowlist | browser CORS disabled | `--cors-origins` flag |
| `STACKKITS_ALLOW_WILDCARD_CORS` | Explicit local-only wildcard CORS override | `false` | `--allow-wildcard-cors` flag |
...
# Dev environment
mise run dev                            # run the CLI entrypoint
docker compose up -d --build            # start local compose services when needed
...
make test-cue-binding                   # CUE contracts + Go binding
for d in modules/*/tests/integration_test.sh; do bash "$d"; done
