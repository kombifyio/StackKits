...
| Target context | legacy v1 `context` field | Migration input only. Native v2 `init` and `generate` do not accept `--context`; `local`/`cloud`/`pi` map to site kind and hardware profile. |
