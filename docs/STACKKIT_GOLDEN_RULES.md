...
8. A release is invalid if the default identity path can generate empty owner, password, session-secret, or TinyAuth user values. Identity credentials must be generated from CUE/module contracts before `apply`, and tests must assert non-empty runtime values without checking secrets into source control.
