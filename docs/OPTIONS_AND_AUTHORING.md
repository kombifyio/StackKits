...
| Mail server | `stalwart` | Own mail server on the Cloud Kit only (ADR-0046); home nodes are refused until an outbound relay input exists. Publishes TCP 25, 465, 587, 993 and 4190; the route host is the mail host name. `stackkit setup mail-server` creates the domain and first mailbox, prints the DNS records and verifies IMAPS and submission. mailcow is a planned alternative, not declared. |
