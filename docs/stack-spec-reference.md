# StackSpec v1 Migration Reference

> **Status:** Historical StackSpec v1 input; migration/read-only compatibility.
> **Format:** YAML
> **Purpose:** Interpret existing installations before explicit migration.

New installations use `apiVersion: stackkit/v2alpha2`. Start with
[`stackkit init` and the native configuration guide](CONFIGURATION.md#stack-spec).
The current schema is CUE's `#StackSpecV2` in
[`foundation/architecture_v2.cue`](../foundation/architecture_v2.cue); module
profiles, workloads, topology and observed inventory follow the
[Architecture v2 contract](ARCHITECTURE.md). The examples below are retained
only for reading old configuration. They must not be used to author a new stack
or to bypass the native lifecycle.

## Overview

Older installations used this unversioned shape in `stack-spec.yaml`, with
`kombination.yaml` accepted as a read alias. Native generation and execution do
not silently reinterpret this legacy input as Architecture v2.

Existing files may have been exported by Techstack or authored through an older
CLI. Current `stackkit init` writes the native versioned schema instead.
